...
class SpriteSheet(object):
    def __init__(self, filename, patch_size, final_size):
        self._sheet_surf = pygame.image.load(filename).convert_alpha()
    ...
    def patch2pixel(self, patch_pos):
        ...
        """
        left = self._patch_size[0] * patch_pos[0]
        top  = self._patch_size[1] * patch_pos[1]
        return left, top
    ...
    def get_patch(self, patch_pos):
        ...
        patch = pygame.Surface(self._patch_size)
        ...
        patch.set_colorkey(FLOOR_COLOUR)
        patch = pygame.transform.scale(patch, self._final_size)
